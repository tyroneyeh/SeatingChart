 ...
 */
const interpolate = function (plugin) {
    return function (msgid, context, disableHtmlEscaping, parent) {
        ...
        const result = msgid.replace(INTERPOLATION_RE, function (_match, token) {
            ...
            function getProps(obj, expression) {
                const arr = expression.split(EVALUATION_RE).filter(function (x) { return x; });
                while (arr.length) {
                    obj = obj[arr.shift()];
                }
                return obj;
...
const translate = function (language) { return ({
    /*
    ...
    getTranslation: function (msgid, n, context, defaultPlural, languageKey, parameters, disableHtmlEscaping) {
        ...
        const interp = function (message, parameters) {
        ...
        const noTransLangKey = languageKey; // if no translation, use current lang key to get plural text.(for compatibility)
        ...
        const getTranslationFromArray = function (arr) {
            const translationIndex = plurals.getTranslationIndex(languageKey, n);
        ...
        const getUntranslatedMsg = function () {
            if (!silent) {
                const msg = "Untranslated ".concat(languageKey, " key found: ").concat(msgid);
                if (context) {
        ...
        const translateMsg = function (msg, context) {
            if (context === void 0) { context = null; }
...
const Component = vue.defineComponent({
    ...
    setup: function (props, context) {
        ...
        const translation = vue.computed(function () {
            let _a;
...
const updateTranslation = function (language, el, binding, vnode) {
    let _a;
...
function createGettext(options) {
    ...
    const mergedOptions = __assign(__assign({}, defaultOptions), options);
    ...
    const gettext = vue.reactive({
        ...
        install: function (app) {
            ...
            if (mergedOptions.setGlobalProperties) {
                const globalProperties_1 = app.config.globalProperties;
...
const defineGettextConfig = function (config) {
    return config;
};
